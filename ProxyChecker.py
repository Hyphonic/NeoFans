...
async def Show(Proxies):
    while True:
        try:
            ...
            Logger.Info(f'Found Proxy: {Proxy.types.lower()}://{Proxy.host}:{Proxy.port}')
            
            with open(f'{Proxy.types.lower()}.txt', 'a') as File:
                File.write(f'{Proxy.types.lower()}://{Proxy.host}:{Proxy.port}\n')
